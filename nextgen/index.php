...
<head>
  ...
  <style>
    ...
    * {
    ...
    margin-left: 20px; /* Add margin to the left to move it to the right */
    }
    ...
    .dropdown li a:hover {
        background: #e3f2fd; /* Change background on hover */
    }
    ...
    .shop-now-btn:hover {
        transform: translateY(4px); /* Move the button down by 4 pixels on hover */
    }
    .banner {
      position: relative;
        ...
        <div class="language-selector">
            <form method="POST" action="">
                <select name="language" onchange="this.form.submit()">
                    ...
                    <option value="ta" <?php if ($_SESSION['lang'] === 'ta') echo 'selected'; ?>>தமிழ்</option>
                </select>
            </form>
        </div>
    </div>
    ...
    <div id="products">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<p><strong>Discount Price: ₹" . number_format($row['discount_price'], 2) . "</strong></p>";
                echo "<img src='" . htmlspecialchars($row['image_url']) . "' class='product-image' alt='" . htmlspecialchars($row['product_name']) . "' />";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    // Redirect back to homepage or wherever appropriate
    header("Location: index.php"); // Change to the correct page if needed
    exit;
}
?>
